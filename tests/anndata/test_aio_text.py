# -*- coding: UTF-8 -*-
"""
:filename: sppas.src.anndata.tests.test_aio_text.py
...
from sppas.src.anndata.aio.text import sppasRawText
from sppas.src.anndata.aio.text import sppasCSV
from sppas.src.anndata.aio.text import sppasBaseText
...
class TestRawText(unittest.TestCase):
    ...
    def test_members(self):
        ...
        self.assertFalse(txt.empty_tier_support())
        self.assertFalse(txt.metadata_support())
        self.assertTrue(txt.comments_support())
        self.assertFalse(txt.ctrl_vocab_support())
        self.assertFalse(txt.media_support())
        self.assertFalse(txt.hierarchy_support())
        self.assertTrue(txt.point_support())
    ...
    def test_read1(self):
        ...
        self.assertEqual(len(txt[0]), 9)  # was 6
...
class TestCSVText(unittest.TestCase):
    ...
    def test_write_read_unsupported(self):
        """CSV holds in a tier what it can't hold otherwise."""
        if os.path.exists(TEMP) is False:
            os.mkdir(TEMP)
        output = os.path.join(TEMP, "sample-unsupported.csv")
        ...
        # CSV can't hold a whitespace in such a tier: it is an underscore
    ...
    def test_format_lines(self):
        ...
        csv = sppasCSV()
        lines = list()
        lines.append('"transcript"\t"7.887"\t"10.892"\t"Go maith anois a mhac. Anois cé acub?"'.split("\t"))
        lines.append('"transcript"\t"11.034"\t"12.343"\t"Tá neart ábhair ansin anois agat."'.split("\t"))
    ...
    def test_read_write(self):
        txt = sppasCSV()
        txt.read(os.path.join(DATA, "sample-irish.csv"))
        txt.write(os.path.join(DATA, "sample-irish-test.csv"))

        txt = sppasCSV()
        txt.read(os.path.join(DATA, "sample-irish-test.csv"))
        self.assertEqual(len(txt), 2)
        self.assertEqual(len(txt[0]), 5)
        self.assertEqual(len(txt[1]), 5)
        os.remove(os.path.join(DATA, "sample-irish-test.csv"))
