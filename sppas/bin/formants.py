...
# -*- coding: UTF-8 -*-
"""
:filename: sppas.bin.formants.py
:author: Brigitte Bigi
:contact: contact@sppas.org
:summary: Run the Formants estimator(s) on phonemes' intervals.
...
from sppas.src.annotations import sppasFormants
...
def get_args_from_cmd(parameters, ann_step_idx):
    ...
    group_io.add_argument(
        "-o",
        metavar="file",
        help='Annotated file with F1/F2 formants values '
             '(default: None)')
...
def formants():
    ...
    arguments = vars(args)
    for a in arguments:
        if a not in ('W', 'i', 'o', 't', 'I', 'e', 'quiet', 'log'):
