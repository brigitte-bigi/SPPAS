...
# ---------------------------------------------------------------------------
#
#       Copyright (C) 2017-2025  Brigitte Bigi, CNRS
...
if args.P and len(args.P) > 0 and args.P[0] == "-":
    args.P = args.P[1:]
...
try:
    ...
    if not args.quiet:
        print(f"Loaded audio successfully: {args.i}")
...
if not args.quiet:
    print(f"Loaded annotated data file successfully: {ann_file}")
...
if args.video:
    ...
    print(sppasFiles.get_informat_extensions("VIDEO"))
    for ext in sppasFiles.get_informat_extensions("VIDEO"):
        _tmp = filename + vpattern + ext
...
if vid_file is not None:
    ...
    if not args.quiet:
        print(f"Loaded video file successfully: {vid_file}")
...
for i, ann in enumerate(tier):
    ...
    logging.debug(f"- Extract track: [{begin}, {end}]")
