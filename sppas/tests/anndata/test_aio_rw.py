# -*- coding: UTF-8 -*-
"""
:filename: sppas.tests.anndata.test_aio_rw.py
...
def compare_tiers_trs(trs1, trs2, meta=True):
    ...
    for t1, t2 in zip(trs1, trs2):
        if t1.get_name() != t2.get_name():
            ...
            return False
        if len(t1) != len(t2):
            print('Tier {:s}, sizes differ: {:d} != {:d}'.format(t1.get_name(), len(t1), len(t2)))
            return False
        if meta is True:
            # compare metadata
...
class TestAIO(unittest.TestCase):
    ...
    def test_IO_XRA(self):
        ...
        # Write XRA file
        parser.set_filename(os.path.join(TEMP, "sample-1.5.xra"))
        parser.write(trs1)
    ...
    def test_IO_CTM(self):
        """Read/Write/Read then compare CTM files."""
    ...
    def test_IO_STM(self):
        """Read/Write/Read then compare STM files."""
    ...
    def test_IO_SRT(self):
        """Read/Write/Read then compare SRT files."""
    ...
    def test_IO_SUB(self):
        """Read/Write/Read then compare SUB files."""
    ...
    def test_IO_TXT(self):
        """Read/Write/Read then compare TXT files."""
        ...
        #self.assertTrue(compare_tiers_trs(trs1, trs2, meta=False))  # The 'id' differ...
    ...
    def test_IO_CSV(self):
        """Read/Write/Read then compare CSV files."""
    ...
    def test_IO_TextGrid_CSV(self):
        """Read/Write/Read then compare TextGrid-CSV files."""
        ...
        # Read the file
        parser = sppasTrsRW(os.path.join(TEMP, "sample.csv"))
        trs2 = parser.read(heuristic=True)

        # compare (meta is False because 'id' is different)
        self.assertTrue(compare_tiers_trs(trs1, trs2, meta=False))
