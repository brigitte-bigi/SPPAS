# -*- coding: UTF-8 -*-
"""
:filename: sppas.src.anndata.tests.test_tier.py
:author: Brigitte Bigi
:contact: contact@sppas.org
:summary: Test sppasTier() class.
...
from sppas.src.anndata.anndataexc import CtrlVocabContainsError
...
class TestTier(unittest.TestCase):
    ...
    def test_split(self):
        ...
        self.assertEqual(6, a.get_lowest_localization())
        self.assertEqual(9, a.get_highest_localization())

        # Normal situation, but not enough location duration to be split
        with self.assertRaises(IntervalBoundsError):
            tier.split(0)

        # Split with "from" value: split at this time value instead of middle
        a3 = sppasAnnotation(sppasLocation(sppasInterval(sppasPoint(10), sppasPoint(15))))
        idx = tier.add(a3)
        with self.assertRaises(ValueError):
            a = tier.split(idx, 7)
        with self.assertRaises(ValueError):
            a = tier.split(idx, 10)
        a = tier.split(idx, 13)
        self.assertEqual(10, a3.get_lowest_localization())
        self.assertEqual(13, a3.get_highest_localization())
        self.assertEqual(13, a.get_lowest_localization())
        self.assertEqual(15, a.get_highest_localization())

        # Split with "from-to" values
        a4 = sppasAnnotation(sppasLocation(sppasInterval(sppasPoint(20), sppasPoint(30))))
        idx = tier.add(a4)
        with self.assertRaises(ValueError):
            a = tier.split(idx, 25, 45)
        with self.assertRaises(ValueError):
            a = tier.split(idx, 25, 18)
        a = tier.split(idx, 24, 26)
        self.assertEqual(20, a4.get_lowest_localization())
        self.assertEqual(24, a4.get_highest_localization())
        self.assertEqual(26, a.get_lowest_localization())
        self.assertEqual(30, a.get_highest_localization())
    ...
    def test_find_interval(self):
        ...
        annotations = tier.find(sppasPoint(0), sppasPoint(5), overlaps=False, radius_overlaps=False)
        self.assertEqual(len(tier), len(annotations))
        ...
        annotations = tier.find(sppasPoint(0.5), sppasPoint(5), overlaps=True)
        self.assertEqual(len(annotations), len(tier))
        for x, y in zip(tier, annotations):
            self.assertTrue(x is y)
        ...
        t1 = sppasTier("test-with-overlapping-radius")
        ...
        annotations = t1.find(sppasPoint(2.49), sppasPoint(6.49), overlaps=False)
        self.assertEqual(annotations, [])
        annotations = t1.find(sppasPoint(2.49), sppasPoint(6.49), overlaps=True)
        self.assertEqual(annotations, [])
    ...
    def test_find_overlapped_intervals_radius(self):
        ...
        localizations = [sppasInterval(sppasPoint(1.), sppasPoint(2.)),    # 0
                         ...
                         sppasInterval(sppasPoint(1.8), sppasPoint(2.5)),  # 3
                         sppasInterval(sppasPoint(2.), sppasPoint(2.3)),   # 4
                         sppasInterval(sppasPoint(2.), sppasPoint(2.5, 0.2)),   # 5
                         sppasInterval(sppasPoint(2.), sppasPoint(3.)),    # 6
                         sppasInterval(sppasPoint(2.4), sppasPoint(4.)),   # 7
                         sppasInterval(sppasPoint(2.5), sppasPoint(3.))    # 8
                         ]
        ...
        t2 = sppasTier("test2-with-overlapping-radius")
        a0 = t2.create_annotation(sppasLocation(sppasInterval(sppasPoint(0.), sppasPoint(1.4))))
        a1 = t2.create_annotation(sppasLocation(sppasInterval(sppasPoint(1.4), sppasPoint(2.4, 0.4))))
        a2 = t2.create_annotation(sppasLocation(sppasInterval(sppasPoint(2.4, 0.4), sppasPoint(6.1, 0.))))
        a3 = t2.create_annotation(sppasLocation(sppasInterval(sppasPoint(6.6, 0.6), sppasPoint(8.))))
        a4 = t2.create_annotation(sppasLocation(sppasInterval(sppasPoint(8.), sppasPoint(10.))))
        annotations = t2.find(sppasPoint(2.49), sppasPoint(6.49), overlaps=False, radius_overlaps=True)
        self.assertEqual(annotations, [a2])
        annotations = t2.find(sppasPoint(2.49), sppasPoint(6.49), overlaps=True, radius_overlaps=True)
        self.assertEqual(3, len(annotations))
        self.assertEqual(annotations, [a1, a2, a3])

        t1 = sppasTier("test1-with-overlapping-radius")
        a1 = t1.create_annotation(sppasLocation(sppasInterval(sppasPoint(0.), sppasPoint(2.4, 0.4))))
        a2 = t1.create_annotation(sppasLocation(sppasInterval(sppasPoint(6.6, 0.6), sppasPoint(8.))))
        annotations = t1.find(sppasPoint(2.49), sppasPoint(6.49), overlaps=False, radius_overlaps=True)
        self.assertEqual(annotations, [])
        annotations = t1.find(sppasPoint(2.49), sppasPoint(6.49), overlaps=True, radius_overlaps=True)
        self.assertEqual(2, len(annotations))
        self.assertEqual(annotations, [a1, a2])
