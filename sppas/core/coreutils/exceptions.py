"""
:filename: sppas.src.config.exceptions.py
...
class sppasPythonError(Exception):
    """:ERROR 9999: Deprecated Python: SPPAS requires 2.7+.

    """

    def __init__(self):
        self._status = 9999
        self.parameter = ":ERROR 9999: Invalid Python. SPPAS requires 2.7+."
...
class sppasPythonFeatureError(Exception):
    ...
    def __init__(self, fid, pyversion):
        ...
        self.parameter = error(self._status) + \
                         (error(self._status, "globals")).format(fid, pyversion)
...
class sppasSystemOSError(sppasOSError):
    """:ERROR 0505:.
...
class LanguageNotFoundError(sppasValueError):
    """:ERROR 0350:.
    ...
    def __init__(self, code):
        super(LanguageNotFoundError, self).__init__("", 0)
        self._status = 350
        self.parameter = error(self._status) + \
                         (error(self._status, "globals")).format(code=code)
...
class sppasReadError(IOError):
    ...
    def __init__(self, filename, msg="unknown"):
        self._status = 655
        self.parameter = error(self._status) + \
                         (error(self._status, "globals")).format(name=filename, error=msg)
