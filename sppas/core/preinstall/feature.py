# -*- coding : UTF-8 -*-
"""
:filename: sppas.core.preinstall.feature.py
:author: Brigitte Bigi
:contributor: Florian Hocquet
:contact: contact@sppas.org
:summary: Data structure for one of the features of SPPAS dependencies.
...
def _msg(entry):
    return msg(entry, "globals")

MSG_NO_BRIEF = _info(590)
MSG_NO_DESCR = _info(592)
...
class Feature(object):
    ...
    def __init__(self, identifier):
        ...
        # Represent if the feature is enabled
        self.__enable = False
...
class DepsFeature(Feature):
    """Store information of one dependency feature required by the application.

    Subclass to represent a package to be installed or a command to be executed,
    i.e., the need of external programs or Python modules.
    ...
    def __init__(self, identifier):
        ...
        # Names of Python modules to check, and commands to check
        self.__pip_test = str()
        self.__cmd_test = str()
    ...
    def set_cmd_test(self, value):
        """Set the command line to test availability of the feature."""
        self.__cmd_test = str(value).strip()

    # ------------------------------------------------------------------------

    def get_cmd_test(self):
        """Return the test command to check feature availability."""
        return self.__cmd_test
    ...
    def set_cmd(self, value):
        ...
        value = str(value)
        if "$PY" in value:
            value = value.replace("$PY", sys.executable)
        if "$SPPAS" in value:
    ...
    def get_pypi_packages(self):
        """Return the list of pip packages names."""
        return [key for key in self.__pypi]

    # ------------------------------------------------------------------------

    def get_pypi_package_version(self, package_name):
        """Return the version constrains of a given pip package.

        :param package_name: (str)
        :return: (str) package version or empty string

        """
        return self.__pypi.get(package_name, "")

    # ------------------------------------------------------------------------

    def add_pypi(self, package_name, package_version):
        """Add a pip package in the pip dependencies.

        :param package_name: (str)
        :param package_version: (str)

        """
        package_name = str(package_name)
        package_version = str(package_version)
        self.__pypi[package_name] = package_version

    # ------------------------------------------------------------------------

    def get_pypi_alt_packages(self):
        """Return the list of pip packages names."""
        return [key for key in self.__pypi_alt]

    # ------------------------------------------------------------------------

    def get_pypi_alt_package_version(self, package_name):
        """Return the version constrains of a given pip package.
        ...
        """
        return self.__pypi_alt.get(package_name, "")

    # ------------------------------------------------------------------------

    def add_pypi_alt(self, package_name, package_version):
        """Add a pip alternative package in the pip dependencies.
        ...
        package_name = str(package_name)
        package_version = str(package_version)
        self.__pypi_alt[package_name] = package_version
    ...
    def get_pip_test(self):
        """Return the pip_test field, i.e., the names of Python modules to check.

        :return: (str) The value of pip_test, or an empty string if not set.

        """
        return self.__pip_test
...
class LangFeature(Feature):
    ...
    Subclass to represent a language support, ie the need to download
...
class AnnotFeature(Feature):
    ...
    Subclass to represent the support of an automatic annotation, ie
...
class SpinOffFeature(Feature):
    """Store information of one spin-off feature of the application.

    Subclass to represent a spin-off component, developed separately
    from SPPAS but using the same architecture, and which can be integrated
    into SPPAS to add or extend functionality (usually annotation).

    """

    def __init__(self, identifier):
        super(SpinOffFeature, self).__init__(identifier)
        self.__spin = str()
        self.__url = str()
        self.__version = str()
    ...
    def get_spin(self):
        """Return the spin-off resource name to be integrated."""
        return self.__spin

    # ------------------------------------------------------------------------

    def set_spin(self, value):
        """Set the spin-off resource name to be integrated.

        :param value: (str) Name of the spin-off component (ASCII only)

        """
        value = str(value).strip()
        if len(value) == 0:
            logging.warning("Attempted to set an empty spin-off name for "
                            "feature '{}'.".format(self.get_id()))
        else:
            self.__spin = value

    # ------------------------------------------------------------------------

    def get_spin_url(self):
        """Return the spin-off url, i.e. the place where to find it."""
        return self.__url

    def set_spin_url(self, value):
        """Set the spin-off url, i.e. the place where to find it.

        :param value: (str) Name of the spin-off url

        """
        value = str(value).strip()
        if "http" not in value:
            logging.warning("Attempted to set an invalid spin-off url for "
                            "feature '{}'.".format(self.get_id()))
        else:
            self.__url = value
    ...
    def get_spin_version(self):
        """Return the spin-off version."""
        return self.__version

    def set_spin_version(self, value):
        """Set the spin-off version.

        :param value: (str) spin-off version, like "1.0".

        """
        value = str(value).strip()
        if len(value) == 0:
            logging.warning("Attempted to set an invalid version for "
                            "feature '{}'.".format(self.get_id()))
        else:
