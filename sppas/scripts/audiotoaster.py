...
import audioopy.aio
from audioopy.aio import extensionsul
...
from sppas.ui.term import ProcessProgressTerminal
...
def extract_channels(settings, factor=0, channel=0, p=None):
    ...
    for i, channelparameter in enumerate(settings):
        ...
        audio = audioopy.aio.open(channelparameter['file'])
        ...
        if factor > 0:
            ...
            mel = AudioConverter().hz2mel(rms)
            mel_wanted = mel * factor
            rms_wanted = AudioConverter().mel2hz(mel_wanted)
...
def mix_channels(settings, p=None):
    ...
    mixer_left = ChannelMixer()
    mixer_right = ChannelMixer()
...
maxvalth = AudioFrames().get_maxval(sampleswidth)
