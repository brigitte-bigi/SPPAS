...
from .modelmixer import sppasModelMixer
...
class sppasAlign(sppasBaseAnnotation):
    ...
    def _segment_tracks(self, workdir):
        ...
        # Store the duration of each track --
        durations = list()
        with open(os.path.join(workdir, "tracks.list"), "r") as fp:
            for line in fp:
                v1, v2 = line.split(" ")
                durations.append(float(v2) - float(v1))
        ...
        while track_number < nb_tracks:
            ...
            try:
                if os.path.exists(audio) is False:
                    if track_number < len(durations):
                        msg = self._segmenter.segment(durations[track_number-1], phn, token, align)
                    else:
                        msg = self._segmenter.segment(None, phn, token, align)
                else:
                    msg = self._segmenter.segment(audio, phn, token, align)
                if len(msg) > 0:
                ...
                self.logfile.print_message(
                    MSG_ALIGN_FAILED.format(name=self._segmenter.get_aligner_name()),
                    indent=2,
                    status=annots.error)
                ...
                if self._options['basic'] is True:
                    self.logfile.print_message(
                        "Segment is rescued by assigning the same duration to each phoneme.",
    ...
    def run(self, input_files, output=None):
        ...
        if audio_filename is not None:
            ...
            msg = ("Either the audio file or 'AudiooPy' library is unavailable. "
                   "The automatic aligner is set to 'basic' and no extra option is available.")
            logging.warning(msg)
            if self.logfile is not None:
                self.logfile.print_message(msg, indent=1, status=annots.warning)
