...
from sppas.core.coreutils import basestring
...
class HtkModelSemantics(object):
    ...
    def _default(self, ast):
        ...
        except AttributeError:   # Pythhon 10.x
            if isinstance(ast, collections.abc.Mapping):
                return _to_ordered_dict(ast)
        return ast
...
class HtkModelParser(Parser):
    ...
    def _basekind_(self):
        with self._choice():
            ...
            with self._option():
                self._token('user')
            self._error('expecting one of: discrete fbank cuedspeech lpcepstra lpdelcep lprefc melspec mfcc user')
