# -*- coding: UTF-8 -*-
"""
:filename: sppas.src.annotations.Align.models.acm.htktrain.py
:author:   Brigitte Bigi
:contact:  contact@sppas.org
:summary:  Training procedure of an HTK acoustic model.
...
from audioopy.aio import save as audio_save
...
from sppas.src.anndata.aio.aioutils import serialize_labels, unalign
...
from .channelmfcc import sppasChannelMFCC
from .acmodel import sppasAcModel
from .acmodelhtkio import sppasHtkIO
from .acfeatures import sppasAcFeatures
...
def test_command(command):
    ...
    """
    return sppasExecProcess().test_command(command)
    # try:
...
class sppasTrainingCorpus(object):
    ...
    Accepted input:
        ...
        - audio files: one of audioopy.aio.extensions
    ...
    def add_corpus(self, directory):
        ...
        trs_files = list()
        for extension in sppasTrsRW.extensions_in():
    ...
    def _append_phonetization(self, tier, trs_filename, audio_filename):
        ...
        for ann in tier:
            label = serialize_labels(ann.get_labels(), separator=" ", empty="", alt=False)
    ...
    def _add_audio(self, audio_filename, outfile):
        ...
        try:
            audio = audio_open(audio_filename)
        ...
        audio_save(os.path.join(self.datatrainer.get_storewav(), outfile + ".wav"), audio_out)
...
class sppasHTKModelTrainer(object):
    ...
    def init_epoch_dir(self):
        ...
        if self.__current_dir is not None:
            macro_src = os.path.join(self.__current_dir, DEFAULT_MACROS_FILENAME)
            if os.path.exists(macro_src):
                macro_dst = os.path.join(next_dir, DEFAULT_MACROS_FILENAME)
                shutil.copyfile(macro_src, macro_dst)
        ...
        try:
            ...
            if test_command("julius") is False:
                if test_command("HVite") is True:
                    ...
                    raise Exception("None of Julius nor HVite command is available.")
            else:
            ...
            return tokenizer, phonetizer, aligner
    ...
    def align_trs(self, tokenizer, phonetizer, aligner):
        ...
        for trs_filename, trs_workfile in self.corpus.transfiles.items():
            ...
            if tier_input is None:
                ...
                if tier_input is None:
                    logging.info(" ... [ERROR] No transcription tier for file: {:s}"
                                 "".format(trs_workfile))
                    continue
            ...
            try:
                tier_phn, tier_tok, tier_pron = \
                    aligner.convert(phones_tier, tokens_faked, None, audio_work_file, alignerdir)
                """                
                ...
                logging.info(" ... [ERROR] Alignment error failed file {:s}. {:s}"
                             "".format(trs_workfile, str(e)))
                logging.info(traceback.format_exc())
                logging.debug(trs_filename)
                return False
            ...
            if len(tier_phn) == 0:
                logging.info(" ... [ERROR] Empty result for file {:s}. "
                             "".format(trs_workfile))
                return False
    ...
    def training_step3(self):
        ...
        scp_file = self.corpus.get_scp(aligned=True, phonetized=True, transcribed=False)
        ret = self.train_step(scp_file, dopruning=True)
        if ret is False:
        ...
        try:
            tokenizer, phonetizer, aligner = self.create_annotators()
        except:
            logging.error('Training from aligned data failed.')
            return False

        logging.info(" ... Aligning transcribed files.")
        self.align_trs(tokenizer, phonetizer, aligner)
        ...
        self.align_trs(tokenizer, phonetizer, aligner)  # here we should infer 'sp'
