...
class BasicAligner(BaseAligner):
    ...
    def run_basic(self, duration=None, output_align=None):
        ...
        if duration is None:
            ...
            # Can only work if less than 100 phonemes in 1 second of time
            # -----------------------------------------------------------
            delta = 0.
            if len(phones_list) > 0:
                # estimate delta in centiseconds -- which is a Julius requirement.
                delta = (duration / float(len(phones_list))) * 100.
                if delta < 1.:
                    logging.error(f"Delta time {delta} < 1.0 is too short for basic alignment."
                                  f"It means there are too many phones (> 100) in the unit.")
    ...
    def __gen_index_alignment(self, phonetization, tokenization, phoneslist, phonesdur, output_align=None):
        ...
        timeval = 0
        alignments = []
        for i, phon in enumerate(phoneslist):
            tv1 = timeval
