...
class TrackSegmenter(object):
    ...
    def segment(self, audio, phon_name, token_name, align_name):
        """Call an aligner to perform speech segmentation and manage errors.

        :param audio: (str|float|None) the audio file name of an IPU or its duration or None
        ...
        :raises: IOError: Audio file does not exist
        ...
        if audio is None:
            audio = 100.
        if isinstance(audio, (int, float)) is False and os.path.exists(audio) is False:
            raise IOError(f"Audio file {audio} does not exist.")
        ...
        if isinstance(audio, (float, int)) is True:
            ret = self._basic_aligner.run_alignment(audio, align_name)

        else:
            # Do not align only one phoneme!
            if len(phones.split()) <= 1 and "-" not in phones:
                self._basic_aligner.run_alignment(audio, align_name)
                return ""

            # Execute Alignment
            ret = self._aligner.check_data()
            ret += self._aligner.run_alignment(audio, align_name)
