...
class sppasOtherRepet(sppasBaseRepet):
    ...
    def other_detection(self, inputtier1, inputtier2):
        ...
        while tok_start_src < tok_end_src:
            ...
            if repetition.get_source() is not None:
                ...
                saved = sppasOtherRepet.add_repetition(repetition, inputtier1, inputtier2, tok_start_src, tok_start_echo, trs_output)
                if saved is True:
                    shift = e + 1
    ...
    @staticmethod
    def add_repetition(repetition, spk1_tier, spk2_tier, start_idx1, start_idx2, trs_out):
        ...
        :return: (bool) the repetition was added or not
        ...
        if all_echos_tier is not None:
            for tok_idx in repetition.get_all_echos():
                ...
                # r = sppasLabel(sppasTag("R" + str(or_index + 1)))
                r = ann.get_labels()[0].copy()
                r.set_key("R" + str(or_index + 1))
                find_a = all_echos_tier.find(ann.get_lowest_localization(), ann.get_highest_localization(), overlaps=False)
                if len(find_a) == 1:
                    # Annotation is already existing.
                    _not_found = True
                    for l in find_a[0].get_labels():
                        if l.get_key() == r.get_key() and l.get_best() == r.get_best():
                            _not_found = False
                            break
                    if _not_found is True:
                        find_a[0].append_label(r)
                        find_a[0].set_meta('source_id_of_R{:d}'.format(or_index + 1), src_id)
                else:
                    a = ann.copy()
                    a.set_labels([r])
