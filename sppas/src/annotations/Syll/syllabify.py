...
class Syllabifier(object):
    ...
    def get_rules_filename(self):
        return self.rules.get_filename()
    ...
    def structures_phonetized(self, phonetized_syllable):
        """Return the structure of a phonetized syllable.

        >>> syllable = "a-p-s-k"
        >>> syllabifier.structures_phonetized(syllable)
        >>> "V-C-C-C"

        """
        c = list()
        for p in phonetized_syllable.split(separators.phonemes):
            c.append(self.rules.get_struct(p))
