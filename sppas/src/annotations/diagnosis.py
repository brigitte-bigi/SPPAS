...
anndata_ext = ["." + e for e in sppasTrsRW.extensions_in()]
anndata_ext.extend(["." + e.lower() for e in sppasTrsRW.extensions_in()])
videodata_ext = [e.lower() for e in video_extensions]
imgdata_ext = [e.lower() for e in image_extensions]
with sppasExtensionsSettings() as settings:
    audioopy_ext = [e.lower() for e in settings.AUDIO]
...
class sppasDiagnosis:
    ...
    def check_file(filename):
        ...
        :return: tuple(status identifier, message)
    ...
    def check_audio_file(filename):
        ...
        :return: tuple with (status identifier, message)
