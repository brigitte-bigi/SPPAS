...
class FillIPUs(SearchForIPUs):
    ...
    def _split_into_vol(self):
        ...
        :return: (int) number of ipus really found
    ...
    def fix_threshold_durations(self):
        ...
        :return: (int) nb tracks
        :raises: Exception: Audio undefined
        ...
        if n > self._nb_ipus:
            ...
            while n > self._nb_ipus:
                self._min_sil_dur += self._win_len
                self._min_ipu_dur += self._win_len
                n = self._split_into_vol()
            ...
            while n < self._nb_ipus and \
                    self._min_sil_dur > SearchForIPUs.MIN_SIL_DUR:
                self._min_sil_dur -= self._win_len
                n = self._split_into_vol()
            ...
            if n < self._nb_ipus:
                self._min_sil_dur = p
                while n < self._nb_ipus and \
                        self._min_ipu_dur > SearchForIPUs.MIN_IPU_DUR:
                    self._min_ipu_dur -= self._win_len
                    n = self._split_into_vol()
                ...
                if n < self._nb_ipus:
                    self._min_ipu_dur = m
                    while n < self._nb_ipus and \
                            self._min_sil_dur > SearchForIPUs.MIN_SIL_DUR and \
                            self._min_ipu_dur > SearchForIPUs.MIN_IPU_DUR:
                        self._min_ipu_dur -= self._win_len
                        self._min_sil_dur -= self._win_len
                        n = self._split_into_vol()
