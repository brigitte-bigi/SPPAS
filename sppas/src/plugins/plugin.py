#!/usr/bin/env python
"""
:filename: sppas.src.plugins.plugin.py
:author: Brigitte Bigi
:contact: contact@sppas.org
:summary: Data structure of one plugin of SPPAS.
...
class sppasPluginParam(object):
    ...
    def parse(self):
        ...
        if filename.endswith('json'):
            ...
            if "$PY" in command:
                command = command.replace("$PY", sys.executable)
            if "PLUGIN_PATH" in command:
                command = command.replace("PLUGIN_PATH", self._directory + os.sep)
            if sppasExecProcess().test_command(command) is False:
