"""
:filename: sppas.src.resources.dictpron.py
:author: Brigitte Bigi
:contact: contact@sppas.org
:summary: Pronunciation dictionary resource model.
...
class sppasDictPron:
    ...
    def get_unkstamp(self):
        ...
        If the default symbol is not in the pronunciation dictionary, this
        method returns "sil".

        """
        if self._dict.get(symbols.unk, None) is None:
            return "sil"
        return symbols.unk
    ...
    def get(self, entry, substitution=symbols.unk):
        ...
        :return: Unicode of the pronunciations or the substitution.
    ...
    def get_pron(self, entry):
        ...
        :param entry: (str) A token to find in the dictionary
        :return: Unicode of the pronunciations or the unknown stamp.

        """
        s = sppasDictPron.format_token(entry)
        p = self._dict.get(s, None)
        if p is None:
            return self.get_unkstamp()
        return p
    ...
    def is_pron_of(self, entry, pron):
        ...
        :param entry: (str) An Unicode token to find in the dictionary
        :param pron: (str) An Unicode pronunciation
        :return: (bool)
    ...
    def map_phones(self, map_table):
        ...
        :return: a sppasDictPron instance with mapped phones
