...
class sppasVideoReader(object):
    ...
    def __init__(self):
        ...
        # Options to modify the read image from the video
        self.__rotate = 0.     # Rotation angle
        self.__gray = False    # Turn into grayscale
        self.__alpha = False   # Add/Remove the alpha channel
        self.__rgb = False     # Turn BGR into RGB (not compatible with gray)
    ...
    def open(self, video):
        ...
        logging.debug(" ... Video opened successfully. Test to browse into the {} frames."
                      "".format(self.get_nframes()))
        if test_pos > 0:
            ...
            if success is True:
                ...
                if real_pos != test_pos:
                    logging.warning("Video failed in setting pos: {} != {}"
                                    "".format(real_pos, test_pos))
                    # success = False
        if success is False:
            self.close()
    ...
    def seek(self, value):
        ...
        # far or close seek? eval the diff...
        nb_frames_diff = int(value - self.__video.get(cv2.CAP_PROP_POS_FRAMES))

        # actually, the current position is the right one
        if nb_frames_diff == 0:
            return True

        # the requested value is far away: seek to the position.
        if nb_frames_diff < 0 or nb_frames_diff > 20:
            ...
            # the requested value is very close: grab frames until the position
    ...
    def _preprocess_image_options(self, image):
        ...
        # i.e. RGB(A) values are ranging 0-255
        ...
        if self.__gray is True:
            ...
            # Option: RGB(A) or BGR(A)
            if self.__rgb is True:
                if c == 3:
                    image = image[:, :, [2, 1, 0]]
                elif c == 4:
                    image = image[:, :, [2, 1, 0, 3]]
