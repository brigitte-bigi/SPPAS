# -*- coding: UTF-8 -*-
"""
:filename: sppas.src.wkps.filestructure.py
:author:   Barthélémy Drabczuk, Brigitte Bigi
:contact:  contact@sppas.org
:summary:  Data structures for paths, roots and files.
...
class FileRoot(FileBase):
    ...
    Use instances of this class to hold data related to the root of a file.
    The root of a file is its name without the pattern nor the extension.
    ...
    def pattern(filename):
        ...
        # Use basename to be sure the '-' is not in the path instead of the filename
        base = os.path.basename(os.path.splitext(fn)[0])
        # Search for a valid pattern
        pos = 0
        if '-' in base:
            pos = base.rindex('-')
            # check if length of the pattern is ok (the pattern includes the initial -).
            p = base[pos:]
            ...
            if all_root is True:
                ...
                for new_filename in sorted(glob.glob(self.id+"*")):
                    if os.path.isfile(new_filename) is True:
                        ...
                        # having the same '*' after 'id' does not mean the
                        # files share the same root (its depend on pattern...)
                        frx = FileRoot(fnx.id)
...
class FilePath(FileBase):
    ...
    Use instances of this class to hold data related to the path of
