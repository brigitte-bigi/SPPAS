# -*- coding: UTF-8 -*-
"""
:filename: sppas.src.anndata.tier.py
:author:   Brigitte Bigi
:contact:  contact@sppas.org
:summary:  Represent a tier, i.e. a layer with annotations.
...
class sppasTier(sppasMetaData):
    ...
    def split(self, idx, from_time=None, to_time=None):
        ...
        # Fix "start" time cut point and modify the existing annotation
        ...
        # Create the - next - new annotation and return it
        if to_time is not None:
            cut_point = to_time
    ...
    def find(self, begin, end, overlaps=True, indexes=False, radius_overlaps=False):
        ...
        :param overlaps: (bool) Return also overlapped annotations, using midpoints. \
                  Not relevant for tiers with points.
        :param indexes: (bool) Return indexes instead of annotations
        :param radius_overlaps: (bool) Return also overlapped annotations, \
                  using midpoints and radius. \
                  Not relevant for tiers with points. overlaps must be true.
        ...
        if self.is_point() is True:
            ...
            if lo != -1:
                if overlaps is True:
                    ...
                    # Keep only annotations strictly in the given interval.
                    # No overlapping annotation is allowed.
                    for i, ann in enumerate(self.__ann[lo:]):
                        b = ann.get_lowest_localization()
                        e = ann.get_highest_localization()
                ...
                logging.info("No annotation is matching begin={}, end={}"
                             "".format(begin, end))
    ...
    def __find(self, x, radius_overlaps=False, closest=False):
        ...
        """
        # No annotations in the tier
        if len(self.__ann) == 0:
            return -1
        # x is before the first one
        if x < self.__ann[0].get_highest_localization():
            if closest is False:
                return -1
            return 0
        # x is after the last one
        if x > self.__ann[-1].get_highest_localization():
            if closest is False:
                return -1
            return len(self.__ann) - 1
        # Only one annotation in the tier
        if len(self.__ann) == 1:
            if closest is True:
                return 0
        ...
        # More than one annotation in the tier.
        is_point = self.is_point()
        lo = 0
        hi = len(self.__ann)  # - 1
        mid = int(float(lo + hi) / 2.)

        # Dichotomy search for the ann idx.
        found = False
        ...
        if found is True:
            # The dichotomy search returns randomly an annotation which contains x.
            # The expected result is the first one. So, store all matching ann
            # indexes in a list then get the right one.
            tmp_annotations = list()
            tmp_annotations.append(mid)
            i = mid
            # Go backward into the list of anns, from mid.
    ...
    def __eq__(self, other):
        """Compare annotations ONLY."""
        if other is None:
            return False

        if isinstance(other, sppasTier) is False:
            return False

        if len(self.__ann) != len(other):
            return False

        for a1, a2 in zip(self.__ann, other):
            if a1 != a2:
                return False
