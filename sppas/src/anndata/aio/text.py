...
class sppasRawText(sppasBaseText):
    ...
    def __format_raw_lines(self, lines):
        ...
        # Create faked-IPUs from lines -- Keep also comments
        new_lines = list()
        for line in lines:
        ...
        # Fill-in the tier with IPUs
        n = 1
        for line in new_lines:
            self._create_annotation(tier, n, "#")
            n += 20
        ...
        self._create_annotation(tier, n+20, "#")
...
class sppasCSV(sppasBaseText):
    ...
    def read(self, filename, signed=True):
        ...
        with codecs.open(filename, mode='r', encoding=enc) as fp:
            # Get dialect from Sniffer.
            dialect = csv.Sniffer().sniff(fp.read(1024))
            # Seek to beginning.
            fp.seek(0)

            # reading the CSV file. _reader is an instance of _csv.reader class.
            _reader = csv.reader(fp, dialect)
            for line in _reader:
                lines.append(line)
            logging.debug("{:d} lines were extracted from the CSV file {:s}"
    ...
    def format_columns_lines(self, lines):
        ...
        for i, row in enumerate(lines):
            if len(row) < 4:
                logging.warning("The CSV line {:d} should contains at least 4 "
                                "columns. Got {:d} instead.".format(i+1, len(row)))
                raise AioLineFormatError(i, row)
            ...
            # Add the new annotation. WITH SOME WORSE TEST...
        ...
        with codecs.open(filename, 'w', enc, buffering=8096) as fp:
            ...
            for tier in self._tiers:
                ...
                for ann in tier:
                    ...
                    if point is True:
                        mp = ann.get_lowest_localization().get_midpoint()
                        csvwriter.writerow([name, mp, None, content])
                    else:
                        b = ann.get_lowest_localization().get_midpoint()
                        e = ann.get_highest_localization().get_midpoint()
