# -*- coding: UTF-8 -*-
"""
:filename: sppas.ui.wxapp.page_files.filesviewctrl.py
...
from sppas.src.wkps import FilePath
from sppas.src.wkps import sppasWorkspace
from sppas.ui.wxapp.imgtools import sppasImagesAccess
from sppas.ui.filesext import FileAnnotIcon
...
class FileTreeViewPanel(sppasScrolledPanel):
    ...
    def __update(self):
        ...
        """
        wx.LogDebug("The FileViewCtrl panel is updating the workspace")
...
class FileRootCollapsiblePanel(sppasCollapsiblePanel):
    ...
    COLUMNS = ['state', 'icon', 'file', 'pattern', 'type', 'date', 'size']
    ...
    def set_refs(self, refs_list):
        ...
        if len(txt) > 0:
            refstext.SetMinSize(wx.Size(-1, self.get_font_height()*2))
    ...
    def __create_refstext(self, parent):
        ...
        refs_text.SetMinSize(wx.Size(-1, self.get_font_height()))
        refs_text.Hide()
        return refs_text
    ...
    def __create_listctrl(self, parent):
        ...
        lst.AppendColumn("pattern",
                         format=wx.LIST_FORMAT_LEFT,
                         width=sppasScrolledPanel.fix_size(50))
        lst.AppendColumn("type",
                         format=wx.LIST_FORMAT_LEFT,
                         width=sppasScrolledPanel.fix_size(74))
        lst.AppendColumn("date",
                         format=wx.LIST_FORMAT_RIGHT,
                         width=sppasScrolledPanel.fix_size(130))
        lst.AppendColumn("size",
                         format=wx.LIST_FORMAT_RIGHT,
                         width=sppasScrolledPanel.fix_size(70))
    ...
    def __create_image_list(self):
        ...
        for state in STATES_ICON_NAMES:
            icon_name = STATES_ICON_NAMES[state]
            bitmap = sppasImagesAccess().get_bmp_image(icon_name, icon_size)
        ...
        bitmap = sppasImagesAccess().get_bmp_image("files-file", icon_size)
        il.Add(bitmap)
        self.__ils.append("files-file")

        # The default icon to represent an unknown file
        bitmap = sppasImagesAccess().get_bmp_image("files-unk-file", icon_size)
        il.Add(bitmap)
        self.__ils.append("files-unk-file")

        # The icons of the known file extensions
        for icon_name in FileAnnotIcon().get_names():
            # bitmap = wx.GetApp().get_icon(icon_name, icon_size, "files-file")
            bitmap_filename = sppasImagesAccess().get_icon_filename(icon_name, default="files-file")
            bitmap = sppasImagesAccess().get_bmp_image(bitmap_filename, icon_size)
    ...
    def __update_file(self, fn):
        ...
        if fn.get_id() in self.__fns:
            ...
            pattern = FileRoot.pattern(filename)
            rootname = filename[:len(filename)-len(pattern)]
            self._listctrl.SetItem(index, FileRootCollapsiblePanel.COLUMNS.index("file"), rootname)
            self._listctrl.SetItem(index, FileRootCollapsiblePanel.COLUMNS.index("pattern"), pattern)
        ...
        self._listctrl.SetColumnWidth(2, max(sppasScrolledPanel.fix_size(100), fn_chars*f))
...
class TestPanel(FileTreeViewPanel):
    ...
    def __init__(self, parent):
        ...
        self.Lock([os.path.join(paths.samples, "samples-fra", "F_F_C006_P6.wav")])
