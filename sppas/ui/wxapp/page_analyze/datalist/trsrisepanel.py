# -*- coding: UTF-8 -*-
"""
:filename: sppas.ui.wxapp.page_analyze.datalist.trsrisepanel.py
...
from sppas.ui.wxapp.views import MetaDataEdit
...
class TrsSummaryPanel(sppasFileSummaryPanel):
    ...
    def paste_tier(self, clipboard):
        ...
        # Update the media (perhaps some new ones)
        med = self.FindWindow("media_listctrl")
        med.update(self._trs.get_media_list())
        if len(self._trs.get_media_list()) > 0:
            med.Show()
        else:
            med.Hide()

        # Update the controlled vocab (perhaps some new ones)
        voc = self.FindWindow("vocab_listctrl")
        voc.update(self._trs.get_ctrl_vocab_list())
        if len(self._trs.get_media_list()) > 0:
            voc.Show()
        else:
            voc.Hide()
