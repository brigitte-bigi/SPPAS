"""
:filename: sppas.ui.wxapp.page_editor.listanns.annlabels.py
...
class sppasAnnLabelsCtrl(wx.richtext.RichTextCtrl):
    ...
    def _on_char(self, evt):
        ...
        # if evt.ControlDown() is True and evt.AltDown() is False:
        #     evt.Skip()
    ...
    def __append_styled_char(self, char):
        """Write a character with the appropriate style."""
        self.SetDefaultStyle(self.text_attr)
        if self.__code_edit == "code_xml":
            if char in ('[', '<', '/', ']', '>'):
            ...
            if char in ('{', '}'):
                ...
                self.BeginTextColour(wx.Colour(125, 125, 125))
                self.WriteText(char)
                self.EndTextColour()
            elif char in ('[', ']'):
                self.BeginTextColour(wx.Colour(10, 10, 250))
                self.WriteText(char)
                self.EndTextColour()
            elif char in ('(', ')'):
                self.BeginTextColour(wx.Colour(20, 200, 20))
                self.WriteText(char)
                self.EndTextColour()
            else:
                self.WriteText(char)
