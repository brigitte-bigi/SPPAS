# -*- coding: UTF-8 -*-
"""
:filename: sppas.ui.wxapp.page_editor.editor.py
:author: Brigitte Bigi
:contact: contact@sppas.org
:summary: Main class of the Editor page of the UI based on wxapp.
...
# ---------------------------------------------------------------------------
# List of displayed messages and tooltips:


MSG_FILES = msg("Files: ", "ui")
MSG_OPEN = msg("Open files", "ui")
MSG_SAVE = msg("Save all", "ui")
MSG_CLOSE = msg("Close all", "ui")

TOOLTIP_OPEN = info(9101, "ui")
TOOLTIP_SAVE = info(9111, "ui")
TOOLTIP_CLOSE = info(9121, "ui")
TOOLTIP_SWITCH_VERT = info(9131, "ui")
TOOLTIP_SWITCH_HORIZ = info(9141, "ui")
TOOLTIP_SEARCH = info(9151, "ui")

CLOSE_CONFIRM = info(9050, "ui")
CLOSE_FILE_CONFIRM = info(9060, "ui")
...
class sppasEditorPanel(sppasPanel):
    ...
    def close_files(self, interactive=True):
        """Close the opened files.

        When interactive is True, unlock the closed files and notify parent.
    ...
    def _create_toolbar(self):
        ...
        tb.AddButton("open", text=MSG_OPEN, tooltip=TOOLTIP_OPEN)
        tb.AddButton("save_all", text=MSG_SAVE, tooltip=TOOLTIP_SAVE)
        tb.AddButton("close", text=MSG_CLOSE, tooltip=TOOLTIP_CLOSE)
        tb.AddSpacer(1)

        bd1 = tb.AddButton("way_up_down", tooltip=TOOLTIP_SWITCH_VERT)
        bd1.SetFocusColour(wx.Colour(self.GetForegroundColour()))
        bd2 = tb.AddButton("way_left_right", tooltip=TOOLTIP_SWITCH_HORIZ)
        bd2.SetFocusColour(wx.Colour(self.GetForegroundColour()))
        bd3 = tb.AddButton("search", tooltip=TOOLTIP_SEARCH)
    ...
    def _process_data_changed(self, event):
        ...
        try:
            ...
            wx.LogError('Page Editor: Data were not sent in the event emitted by {:s}'
    ...
    def _process_key_event(self, event):
        ...
        if event.ControlDown() or event.CmdDown():
            ...
            elif key_code == 83 and event.ShiftDown() is False:
                # ctrl+s Save the files
                self.save_files()
                return

            elif key_code == 87 and event.ShiftDown() is False:
                # ctrl+w Close the files
                self.close_files()
                return
...
class TestPanel(sppasPanel):
    ...
    def __init__(self, parent):
        ...
        f3 = os.path.join(paths.samples, "samples-fra", "F_F_B003_P8.wav")
