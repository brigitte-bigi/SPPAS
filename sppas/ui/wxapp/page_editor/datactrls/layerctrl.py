# -*- coding: UTF-8 -*-
"""
:filename: sppas.ui.wxapp.windows.datactrls.layerctrl.py
...
class sppasTierWindow(sppasWindow):
    ...
    It is sent when the tier or one of its annotation is clicked.
    ...
    def get_selected_localization(self):
        ...
        if self.__ann_idx == -1 or self._tier is None:
            return 0., 0.
    ...
    def set_tier(self, tier):
        ...
        if tier is not self._tier:
            ...
            if self.GetName() == "tierctrl":
                self.SetName("tierctrl_" + tier.get_name())

            r = tier.get_meta("private_fg_color_r", "")
            g = tier.get_meta("private_fg_color_g", "")
            b = tier.get_meta("private_fg_color_b", "")
            if len(r)*len(g)*len(b) > 0:
                self.SetForegroundColour(wx.Colour(int(r), int(g), int(b)))

            r = tier.get_meta("private_bg_color_r", "")
            g = tier.get_meta("private_bg_color_g", "")
            b = tier.get_meta("private_bg_color_b", "")
            if len(r) * len(g) * len(b) > 0:
                self.SetBackgroundColour(wx.Colour(int(r), int(g), int(b)))
    ...
    def get_selected_point(self):
        ...
        if self.__point.IsShown():
            return self.__point.get_point().copy()
        return None

    # -----------------------------------------------------------------------

    def set_selected_point(self, point):
        """Set the selected point to the given one or invalidate the current one.

        Notice that it could modify the selected annotation!
        ...
        if point is None:
            ...
            if radius is None:
                self.update_select_annotation(point.get_midpoint())
            else:
                self.update_select_annotation(
                    point.get_midpoint() - radius,
                    point.get_midpoint() + radius
                )

    # -----------------------------------------------------------------------

    def update_select_annotation(self, time_value, time_value2=None):
        """Select the annotation at given (small) time interval."""
        if time_value2 is None:
            time_value2 = time_value

        # Before doing things that takes time, we'll see if this time
        # value is a bound of the currently selected ann.
        if 0 <= self.__ann_idx != -1 < len(self._tier):
            # an annotation is currently selected
            cur_ann = self._tier[self.__ann_idx]
            show_point = self.__fix_pointctrl(cur_ann, time_value, time_value2)
        ...
        ann_idx = self._tier.mindex(time_value, bound=2)
        ...
        if len(anns) == 0:
            ...
            # get the nearest annotation
            ann_idx = self._tier.near(sppasPoint((time_value + time_value2) / 2.),
                                      direction=0)
            if ann_idx != -1:
                anns.append(ann_idx)
    ...
    def move_midpoint(self, old_time_value, time_value):
        ...
        for ann in copied_anns:
            ...
            if old_time_value == begin_point:
                ...
                if self.__point.get_point() == old_time_value and modified_point is None:
                    modified_point = begin_point
            elif old_time_value == end_point:
                end_point = sppasPoint(time_value, end_point.get_radius())
                if self.__point.get_point() == old_time_value and modified_point is None:
                    modified_point = end_point
        ...
        if modified_point is not None:
            self.__point.set_point(modified_point)
            self.notify(action="selected_point_update", value=modified_point)
    ...
    def set_radius(self, midpoint, new_radius):
        """Set the radius value to all points having the given midpoint.

        Notify parent of any change. APPLIED ONLY IF SELECTED ANN.

        :param midpoint: (float)
        :param new_radius: (float)
        :return: (sppasPoint or None)
        ...
        for ann_idx in self.__point_anns:
            ...
            begin_point = ann.get_lowest_localization()
            end_point = ann.get_highest_localization()
            local_modified_point = None
            if midpoint == begin_point:
                begin_point.set_radius(new_radius)
                local_modified_point = begin_point
            if midpoint == end_point:
                end_point.set_radius(new_radius)
                local_modified_point = end_point
            try:
                ann.set_best_localization(sppasInterval(begin_point, end_point))
            except Exception as e:
                wx.LogError(str(e))
                ...
                self.notify(action="ann_update", value=ann_idx)
                if modified_point is None:
                    modified_point = local_modified_point
    ...
    def OnMouseDragging(self, event):
        ...
        if self._state[1] == WindowState().selected and not self.__infos:
            w, h = self.GetSize()
            ...
            self.__drag_window.SetSize(wx.Size(abs(ew), h))
        ...
        if self._state[1] == WindowState().selected:
            if self.__selected:
                ...
                if self.__drag is True:
                    # A dragging was performed. Aims to create a new annotation.
                    ...
                    # Create an annotation corresponding to the dragged windows. If drag width
                    # is not large enough, do not consider it a drag but a click.
                    if self._tier is not None and ww > 20:
                        start_midpoint = self.__period[0] + self._eval_sec(xw)
                        end_midpoint = self.__period[0] + self._eval_sec(xw + ww)
                        try:
                            # Adjust radius with pxsec (the higher zoom, the smaller radius)
                            radius = max(0.001, round(self._eval_sec(3), 3))
                            loc = sppasInterval(sppasPoint(start_midpoint, radius),
                                                sppasPoint(end_midpoint, radius))
                ...
                if self._tier is not None:
                    # A click was performed. Aims to select an annotation or a boundary.
                    old_point = self.__point.get_point()
                    ...
                    self.update_select_annotation(time_value1, time_value2)
                    if old_point != self.__point.get_point():
                        self.notify("selected_point_update", self.__point.get_point())
                else:
            ...
            # test self, in case self was destroyed in the event handler
            if self:
                event.Skip()
    ...
    def DrawBackground(self, dc, gc):
        ...
        if self._tier is None:
            x, y, w, h = self.GetContentRect()
            try:
                color = wx.GetApp().settings.bg_color
            except AttributeError:
                color = wx.WHITE
            brush = wx.Brush(color, wx.BRUSHSTYLE_SOLID)
            dc.SetBrush(brush)
        ...
        c0 = self.GetPenBackgroundColour()
        c1 = c0.ChangeLightness(135)
        c2 = c0.ChangeLightness(195)
        mid1 = int(float(h) / 2.)
        # top-mid1 gradient
    ...
    def __DrawNoTier(self, dc, gc):
        """Display no information about the un-defined tier."""
        self.__point.Show(False)
        x, y, w, h = self.GetContentRect()
        tw, th = self.get_text_extend(dc, gc, MSG_NO_TIER)
        self.draw_label(dc, gc, MSG_NO_TIER, x + 200, y + ((h - th) // 2))
    ...
    def __DrawInfos(self, dc, gc):
        ...
        self.draw_label(dc, gc, str(len(self._tier)) + " " + MSG_ANNOTATIONS, x + 200, y + ((h - th) // 2))

        # Draw the index of the selected annotation
        if self.__ann_idx > -1:
            self.draw_label(dc, gc, "(-- {:d} -- {:s})".format(self.__ann_idx + 1, MSG_IS_SELECTED),
                            x + 400, y + ((h - th) // 2))
    ...
    def __DrawPeriodAnnotations(self, dc, gc):
        ...
        if self._pxsec == 0.:
            ...
            ann_indexes = self._tier.find(self.__period[0], self.__period[1],
                                          overlaps=True, indexes=True, radius_overlaps=True)
            for ann_idx in ann_indexes:
                self._DrawAnnotation(dc, gc, ann_idx)

            # The point
            if self.__point.is_dragging() is False:
                # Hide if not in period
                p = self.__point.get_point()
                if p is not None:
                    if p < self.__period[0] or p > self.__period[1]:
                        self.__point.Hide()
                    else:  # re-position
                        x, y, w, h = self.GetContentRect()
                        xp, wp = self.xw_point(self.__point.get_point())
                        self.__point.SetSize(wx.Size(wp, h))
                        # to get the exact position in screen, I've to shift of 1 px.
                        if wx.Platform == "__WXMAC__" and xp > 0:
                            xp = xp - 1
                        self.__point.SetPosition(wx.Point(xp, y))
                        # Required under MacOS/Linux:
                        self.__point.Refresh(eraseBackground=True)
    ...
    def xw_point(self, point, x=None, w=None):
        ...
        if 0 <= self.__ann_idx < len(self._tier):
    ...
    def _setup_events(self) -> None:
        ...
        self.Bind(event=sppasEVT_POINT_MOVED, handler=self._process_point_moved)
        self.Bind(event=sppasEVT_POINT_RESIZED, handler=self._process_point_resized)
    ...
    def _on_size(self, event):
        ...
        # Re-draw only if the new pxsec is different
    ...
    def _process_point_moved(self, event):
        ...
        if self._tier is None:
            return
        # There's only one pointctrl in this class... so the event comes from it.
        # Evaluate the new midpoint value
        px, _ = event.pos
        pw, _ = event.size
        old_time_value = self.__point.get_point().get_midpoint()
        time_value = self.__period[0] + self._eval_sec(px + int(round(float(pw)/2., 0)))
        try:
    ...
    def _process_point_resized(self, event):
        ...
        if self._tier is None:
            return
        # There's only one pointctrl in this class... so the event comes from it.
        midpoint = self.__point.get_point().get_midpoint()
        old_time_value = self.__point.get_point().get_radius()
        if old_time_value is None:
            old_time_value = 0.
        ...
        self.set_radius(midpoint, time_value)
        # logging.debug("The pointctrl with radius {:f} was modified to: expected {:f} = observed {:f}"
        #               "".format(old_time_value, time_value, self.__point.get_point().get_radius()))
...
class TestPanel(wx.Panel):
    def __init__(self, parent):
        ...
        self.p0 = sppasTierWindow(self, pos=(10, 10), size=(300, 24), tier=None)
        self.p0.SetBackgroundColour(wx.YELLOW)
        self.p0.show_infos(False)
        self.p0.set_selected_ann(5)
        self.p0.Refresh()

        # show annotations, not information
        aidx = trs[0].near(sppasPoint(5), direction=1)
        trs[0].pop(aidx)
        ...
        # Test when radius is overlapping, but not midpoint. With hole between.
        t1 = sppasTier("test1")
        t1.create_annotation(sppasLocation(sppasInterval(sppasPoint(0.), sppasPoint(2.4, 0.4))))
        t1.create_annotation(sppasLocation(sppasInterval(sppasPoint(6.6, 0.6), sppasPoint(8.))))
        self.p4 = sppasTierWindow(self, pos=(10, 100), size=(300, 64), tier=t1)
        self.p4.set_visible_period(2.49, 6.49)
        self.p4.SetBackgroundColour(wx.Colour(240, 220, 220))
        self.p4.show_infos(False)
        self.p4.Refresh()
        # Test when radius is overlapping, but not midpoint. With ann between
        t2 = sppasTier("test2")
        t2.create_annotation(sppasLocation(sppasInterval(sppasPoint(0.), sppasPoint(2.4, 0.4))))
        t2.create_annotation(sppasLocation(sppasInterval(sppasPoint(2.4, 0.4), sppasPoint(6.1, 0.))))
        t2.create_annotation(sppasLocation(sppasInterval(sppasPoint(6.6, 0.6), sppasPoint(8.))))
        self.p5 = sppasTierWindow(self, pos=(10, 100), size=(300, 64), tier=t2)
        self.p5.set_visible_period(2.49, 6.49)
        self.p5.SetBackgroundColour(wx.Colour(240, 220, 220))
        self.p5.show_infos(False)
        self.p5.Refresh()

        s = wx.BoxSizer(wx.VERTICAL)
        s.Add(btn, 0, wx.EXPAND)
        s.Add(self.p0, 0, wx.EXPAND)
        s.Add(self.p1, 0, wx.EXPAND)
        s.Add(self.p2, 0, wx.EXPAND)
        s.Add(self.p3, 0, wx.EXPAND)
        s.Add(self.p4, 0, wx.EXPAND)
        s.Add(self.p5, 0, wx.EXPAND)
        self.SetSizer(s)
        self.Bind(wx.EVT_COMMAND_LEFT_CLICK, self._process_selected)
        self.Bind(EVT_TIER, self._process_evt_tier)
    ...
    def _process_evt_tier(self, event):
        logging.info("Received EVT_TIER: action={}, value={}"
    ...
    def _switch_view(self, event):
        ...
        self.p4.show_infos(self._show_info)
        self.p5.show_infos(self._show_info)
        self.p1.Refresh()
        self.p2.Refresh()
        self.p3.Refresh()
        self.p4.Refresh()
