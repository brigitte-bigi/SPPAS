"""
:filename: sppas.ui.wxapp.page_editor.timeline.trsview_risepanel.py
...
class TrsViewPanel(sppasFileViewPanel):
    ...
    The object this class is displaying is a sppasTranscription via TrsVista.
    This class is an interface between its parent and the embedded TrsVista.
    ...
    def __init__(self, parent, filename, name="trsview_risepanel"):
        super(TrsViewPanel, self).__init__(parent, filename=filename, name=name)

        # The information we need about the transcription, no more!
        self._ft = TimelineType().trs
        self.Expand()

        # A random light yellow between rgb1 and rgb2
    ...
    def get_selected_tier(self):
        """Return the selected tier or None."""
    ...
    def get_selected_point(self):
        ...
        return self.GetPane().get_selected_point()
    ...
    def update_selected_point(self, point):
        ...
        """
        self.GetPane().update_selected_point(point)
    ...
    def _setup_events(self):
        ...
        """
        self.FindWindow("save").Bind(wx.EVT_BUTTON, self.__process_save_event)

        # Events related to the embedded TrsVista
    ...
    def __process_trs_event(self, event):
        ...
        into an EVT_TIMELINE_VIEW the parent can bind.
    ...
    def __process_save_event(self, event):
...
class TestPanel(sppasScrolledPanel):
    ...
    ZOOMS = (40, 50, 60, 80, 100, 125, 150, 200, 250, 300)
    ...
    def __init__(self, parent):
        ...
        f2 = os.path.join(paths.samples, "samples-fra", "F_F_B003_P8.TextGrid")
        ...
        btn1 = wx.Button(self, size=wx.Size(100, 40), label="Show info/ann")
        btn1.Bind(wx.EVT_BUTTON, self._switch_view)
        self._show_info = True
        btn2 = wx.Button(self, size=wx.Size(50, 40), label="Zoom IN")
        btn2.Bind(wx.EVT_BUTTON, self._zoom_up)
        btn3 = wx.Button(self, size=wx.Size(50, 40), label="Zoom OUT")
    ...
    def _zoom_up(self, event):
        ...
        if next_cur_zoom_idx < len(self.ZOOMS):
            ...
            for child in self.GetChildren():
                if isinstance(child, TrsViewPanel):
                    child.Zoom(self._zoom)
        self.Layout()
        self.SendSizeEvent()
