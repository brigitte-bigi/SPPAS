# -*- coding: UTF-8 -*-
"""
:filename: sppas.ui.wxapp.windows.timeline.audiovista.py
:author:   Brigitte Bigi
:contact:  contact@sppas.org
:summary:  View of an audio file in the timeline.
...
INFOS_LABEL = _("{bits:d} bits, {hz:d}Hz, {seconds:.3f}s, {channels:d} channel(s)")
NO_AUDIO_AVAILABLE_FEATURE_NOT_ENABLED = "No audio available. The {:s} feature is not enabled"
NO_AUDIO_AVAILABLE_UNSUPPORTED_FILE_FORMAT = "No audio available. Is the file format not supported?"
...
class sppasAudioVista(sppasPanel):
    ...
    def UpdateAudioInfos(self, audio_data):
        if self.__waveform is not None and self.__waveform.IsShown():
            self.__waveform.set_audiodata(audio_data)
            self.__waveform.Refresh()

        if self.__infos is not None and self.__infos.IsShown():
            audio_prop = INFOS_LABEL.format(bits=audio_data.get_sampwidth() * 16,
                                            hz=audio_data.get_framerate(),
                                            seconds=audio_data.get_duration(),
                                            channels=audio_data.get_nchannels())
            self.__infos.SetLabel(audio_prop)
    ...
    def get_infos_height(self):
        ...
        """
        h = int(float(sppasAudioVista.INFOS_HEIGHT) * float(self._zoom) / 100.)
        try:
    ...
    def get_waveform_height(self):
        ...
        """
        h = int(float(sppasAudioVista.WAVEFORM_HEIGHT) * float(self._zoom) / 100.)
        try:
            h = sppasPanel.fix_size(h)
        except AttributeError:
            pass
    ...
    def __create_infos_panel(self):
        labeltext = NO_AUDIO_AVAILABLE_FEATURE_NOT_ENABLED.format("audioplay") \
            if not cfg.feature_installed("audioplay") \
            else NO_AUDIO_AVAILABLE_UNSUPPORTED_FILE_FORMAT
        st = wx.StaticText(parent=self, id=-1, label=labeltext, name="infos_panel")
